...
// guarda aqui; nenhuma e inventada, nenhuma e omitida. Chaves opcionais ausentes (undefined) e
// chaves presentes com valor null sao tratadas de forma identica — a fixture grava as duas formas
// de proposito.
...
function evNopat(args) {
  ...
  if (tv === 'gordon') {
    if (roicTv === null || roicTv <= 0 || gp <= -1 || gp >= w) return NaN;
...
function pe(args) {
  ...
  const politicaTv = args.politica_tv ?? 'continua';
  const midYear = args.mid_year ?? false;

  if (g <= -1 || roe <= 0 || ke <= -1 || n < 1) return NaN;
  ...
  if (tv === 'gordon') {
    if (roeTv === null || roeTv <= 0 || gp <= -1 || gp >= ke) return NaN;
...
module.exports = { evNopat, evEbitda, pe, ponteParaPreco, avaliarVetores };
