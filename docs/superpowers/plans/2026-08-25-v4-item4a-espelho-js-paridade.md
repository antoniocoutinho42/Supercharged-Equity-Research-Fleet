...
**D5 — τ como erro relativo com piso absoluto:** `|py − js| ≤ max(τ·|py|, τ)`. Grandezas vão de múltiplos (~10) a valores de equity (~10⁹); um número fixo de casas decimais seria frouxo num extremo e impossível no outro. Valor de τ: **1e-12**. A Task 3 **mede** o erro máximo real, não escolhe uma folga e espera que ela bata: sobre os 440 vetores da fixture o erro relativo máximo é **2.07e-15** (~9 ULP de um double — o ruído esperado de somar ~30 termos com potências), folga de **482×** contra τ. `test_tau_tem_folga_medida` fixa o piso em **100×** (`FOLGA_MINIMA = 100`, pouco mais de duas ordens de grandeza) — não as três ordens que uma versão anterior deste documento exigia sem medir, e que a medição reprovou. 100× deixa espaço para variação entre máquinas e versões do node sem deixar de reprovar um colapso real: um erro de TRANSCRIÇÃO no espelho seria da ordem de 1e-3 ou maior, nove ordens de grandeza acima de τ — nunca perto desta fronteira.
